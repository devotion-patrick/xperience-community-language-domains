...
// XperienceCommunity.LanguageDomains: hostname-based language switching.
// Binds the appsettings section, registers the IPreferredLanguageRetriever /
// IWebPageUrlRetriever decorators (via [assembly: RegisterImplementation] in
// the AssemblyDiscoverable package), and merges hostnames into
// WebsiteChannelDomainOptions.
builder.Services.AddHostnameCultureMapping(builder.Configuration);

// Mirror the same hostname rewriting in the admin URLs tab. Optional but
// recommended; must be the LAST builder.Services.* call before builder.Build()
// so we capture the registration the admin assembly added.
...
// XperienceCommunity.LanguageDomains canonicalises inbound URLs: if a user
// lands on a host other than the language's primary host with a language
// path prefix (e.g. en.example.com/uk/page), 301 them to the canonical
// (uk.example.com/page). Must run BEFORE the path-prefix middleware below.
app.UseMiddleware<HostnameCultureCanonicalRedirectMiddleware>();

// Inbound half of XperienceCommunity.LanguageDomains: translate the
// user-facing display URL into the storage URL Kentico routes against -
// prepend /<langcode> on the host's root language when it isn't the channel
// primary, so Kentico's URL-path routing finds the stored slug. Must run
// BEFORE app.UseKentico().
